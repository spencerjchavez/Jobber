...
export interface PostsState {
  jobPostProps: { [jobPostId: number]: JobPostProps }
  jobCategoryFilter: number[]
}

// Define the initial state using that type
const initialJobPostProps: { [jobPostId: number]: JobPostProps } = {};
for(let i = 0; i < 8; i++) {
  initialJobPostProps[i] = {
    jobPostId: i,
    postDate: Date.now(),
    authorName: 'Jon Doe',
    authorId: 1,
...
export const jobPostsSlice = createSlice({
  ...
  reducers: {
    ...
    removeCategoryFromFilter: (state, action: PayloadAction<number>) => {
      state.jobCategoryFilter = state.jobCategoryFilter.filter((category) => category !== action.payload);
    }
  },
})
